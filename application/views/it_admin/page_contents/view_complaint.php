		...
		<div class="container-fluid">
			...
			<div id="close_complaint" class="modal fade in" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="display: none;">
				<div class="modal-dialog">
					<form action="<?php echo site_url('ITAdmin/close_complaint'); ?>" method="post" class="form-horizontal form-material">
					...
					<div class="card">
						<div class="card-body">
							...
							<?php if ($status != 3 && $dept_name == "IT") { ?>
								...
								<?php } ?>
								<a class="card-link" id="reqButtonToggler" onclick="showDetails('reqButtonToggler', 'productDetails')">
								...
								<?php } ?>
								<button type="button" class="btn btn-info" data-toggle="modal" data-target="#close_complaint">Close
									Complaint
								</button>
							<?php } ?>
						</div>
			...
			<div class="card" id="completionForm" style="display: none;">
				<form onsubmit="return validateCompletionTime()" action="<?php echo site_url('ITAdmin/updateTime/' . $complaint_id); ?>" method="post">
					<div class="card-body">
											...
											<table id="demo-foo-addrow" class="table table-bordered m-t-30 table-hover contact-list">
												...
												<?php foreach ($technicians as $tech) { ?>
													<tr>
														...
														<td><?php echo $tech->technician_id; ?></td>
														<td>
															<form action="<?php echo site_url('ITAdmin/reAssignTechnician'); ?>" method="POST">
																<input type="hidden" name="complaint_id" value="<?php echo $complaint_id; ?>">
																<input type="hidden" name="technician_id" value="<?php echo $tech->technician_id; ?>">
																<button class="btn btn-info d-none d-lg-block m-l-15" type="submit">
																	Assign Task
				...
				<div class="col-12">
					<div class="card">
						<div class="card-body">
							...
							<form action="<?php echo site_url('ITAdmin/addAsset'); ?>" id="product_form">
								<input type="hidden" name="complaint_id" value="<?php echo $complaint_id; ?>">
								<input type="hidden" name="redirect" value="<?php echo site_url('ITAdmin/sendMail'); ?>">
								<div class="table-responsive">
										...
										<table class="table table-bordered m-t-30 table-hover contact-list">
											...
											<?php foreach ($asset_details as $v) { ?>
												<tr>
													...
													<td>
														<form method="post" action="<?php echo site_url('ITAdmin/track_request/' . $v->asset_id); ?>" style="display: inline;">
														...
														<?php if ($v->status > 1) {
															if ($v->reqToTreasurer == 0) { ?>
																<form method="post" action="<?php echo site_url('ITAdmin/req_to_treasurer'); ?>" style="display: inline;">
																	<input type="hidden" name="asset_id" value="<?php echo $v->asset_id; ?>">
																	<input type="hidden" name="complaint_id" value="<?php echo $complaint_id; ?>">
					...
					<div id="content" style="margin: 20px 0px;">
						<ul class="timeline">
							<?php foreach ($complaint_timeline as $d) { ?>
								<li class="event" data-date="<?php echo date('d M Y | g:i A', strtotime($d->date)); ?>">
									<?php if ($d->status == 0) {
										$stats = "Received";
									} else if ($d->status == 1) {
										$stats = "In-Process";
				...
				if ($complaint_feedback == 0) { ?>
					<div class="card">
						<div class="card-body">
							...
							<form method="POST" action="<?php echo site_url('ITAdmin/complaint_feedback/' . $complaint_id); ?>" class="mt-4">
								<div class="form-group">
